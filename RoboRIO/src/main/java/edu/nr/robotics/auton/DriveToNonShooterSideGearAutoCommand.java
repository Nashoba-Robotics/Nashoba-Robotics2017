		...
		if (Robot.side == SideOfField.blue) {
			if (FieldMap.autoTravelMethod == AutoTravelMethod.twoDmotionProfiling) {
				if (FieldMap.gearAlignMethod == GearAlignMethod.camera) {
					...
					addSequential(new MotionProfileToSideGearCommand(FieldMap.FORWARD_DISTANCE_TO_SIDE_PEG.sub(RobotMap.BUMPER_TO_GEAR_DIST), FieldMap.SIDE_DISTANCE_TO_NON_SHOOTER_SIDE_PEG.add(Drive.WHEEL_BASE.mul(0.5)), FieldMap.ANGLE_TO_SIDE_PEG, true));
				}
			} else {
